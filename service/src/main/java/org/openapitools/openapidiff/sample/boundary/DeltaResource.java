...
public class DeltaResource {
    ...
    public Response find(@MultipartForm DeltaInput aInput) throws IOException {
        ...
        return switch(aInput.format) {
            case "html" -> Response.ok(new Markdown2HtmlRender().render(result), MediaType.TEXT_HTML).build();
            case "markdown" -> Response.ok(new MarkdownRender().render(result), MediaType.TEXT_PLAIN).build();
            case "yaml" -> Response.ok(new YamlRender().render(result), MediaType.TEXT_PLAIN).build();
            case "json" -> Response.ok(new JsonRender().render(result), MediaType.APPLICATION_JSON).build();
    ...
    public static class DeltaInput {
        ...
        @Schema(description = "output format", enumeration = {"html", "markdown", "json", "yaml"}, required = true, defaultValue = "html")
        @FormParam("format")
        @DefaultValue("html")
        String format;
